...
export function extractiveDataset(song,sources,excluded,retrievedAt){
 const counts=new Map(),segmenter=new Intl.Segmenter('zh-CN',{granularity:'word'});
 for(const source of sources)for(const token of segmenter.segment(matchedText(source).text)){const term=token.segment;if(!token.isWordLike||term.length<2||term.length>6||stop.has(term)||!/[\u4e00-\u9fff]/u.test(term)||normalize(song.title).includes(normalize(term))||normalize(song.creator||'').includes(normalize(term)))continue;const item=counts.get(term)||{count:0,sourceIds:new Set()};item.count++;item.sourceIds.add(source.id);counts.set(term,item);}
 const terms=[...counts].sort((a,b)=>(b[1].sourceIds.size*3+b[1].count)-(a[1].sourceIds.size*3+a[1].count)||a[0].localeCompare(b[0],'zh-CN')).slice(0,16);
 const keywords=terms.map(([label],i)=>({id:'k'+(i+1),label,description:'在检索摘录中出现的词语',group:'摘录中的表达',frequency:frequency(sources,[label])}));
 const analyses=[];
 for(const keyword of keywords)for(const source of sources){const hit=keyword.frequency.matches.find(m=>m.sourceId===source.id);if(!hit)continue;const quote=quoteAround(matchedText(source).text,hit.offset);analyses.push({sourceId:source.id,keywordId:keyword.id,status:'insufficient',summary:quote.slice(0,45)+(quote.length>45?'…':''),quote,context:'这是匹配词附近的自动截取，不是作者观点总结。可能包含歌词、转述或过去立场；尚未核实说话者及转折，请结合知乎原文判断。',voice:'自动截取 · 立场待核验',summaryKind:'extractive',evidenceTextId:hit.textId,sourceUrl:source.url});}
 return {schemaVersion:2,song:{...song,edition:'实时检索'},preparedAt:retrievedAt,analysisMode:'extractive',keywords,sources,analyses,excluded,limitations:[`${sources.length} 篇文章与回答来自本次搜索，非全文，不代表全部知乎讨论。`,'关键词按所获文本自动提取，观点尚未通过 AI 语境核验；表格显示原句预览，不冒充作者结论。','搜索可能包含同名歌曲或延伸讨论，请核对来源标题与作者。','原文与交流入口仍在知乎；不收录评论。']};
}
...
export function applyGroundedAnalysis(base,output){
 ...
 const usedKeywords=keywords.filter(k=>analyses.some(a=>a.keywordId===k.id));if(usedKeywords.length<2||analyses.length<2)throw Error('通过证据校验的分析不足');
...
export function publicDataset(data){const {excluded,...rest}=data;return {...rest,sources:data.sources.map(({texts,authorVerifiedFrom,...source})=>source)};}
