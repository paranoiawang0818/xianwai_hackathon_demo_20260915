...
export function publicDataset(data){const {excluded,analysisEngine,analysisIssue,...rest}=data;const legacy=data.analysisMode==='extractive';return {...rest,...(analysisIssue?{analysisIssue:{code:analysisIssue.code,stage:analysisIssue.stage,checkedAt:analysisIssue.checkedAt,retryAfter:analysisIssue.retryAfter}}:{}),...(legacy?{analysisMode:'pending',keywords:[],analyses:[],analysisNotice:'这首歌的旧结果尚未完成观点分析，可以复用已有材料重新分析。',limitations:['候选来源来自知乎搜索摘录，非全文；旧版自动分词结果已停用。']}:{}),sources:data.sources.map(({texts,authorVerifiedFrom,...source})=>source)};}
