...
function renderPendingAnalysis(){
 ...
 main.innerHTML=`<div class="page"><div class="wall-breadcrumb"><a href="#search">← 返回搜索</a><span>${escape(data.song.title)} / 已找到的来源</span></div><section class="song-strip"><div><div class="eyebrow">来源已保留</div><h1>${escape(data.song.title)}</h1><p>${escape(data.song.creator||'知乎中的歌曲讨论')}</p></div></section><section class="search-no-result" role="status"><h2>找到了内容，观点分析尚未完成</h2><p>${escape(data.analysisNotice||'本次观点分析尚未完成。你可以先阅读来源，稍后重新分析已有材料。')}</p><div class="search-error-actions"><button class="small-button" id="retry-analysis">重新分析已有材料</button><a class="text-button" href="#search">返回搜索</a></div><p>本次重试不重复搜索，由已配置的分析服务整理知乎摘录。分析通过原句校验后再生成关键词和对照阅读。</p>${data.analysisIssue?`<details><summary>查看本次状态</summary><p>${escape(data.analysisIssue.code)} · ${escape(data.analysisIssue.checkedAt)}</p></details>`:''}</section><section class="source-list" aria-label="已找到的候选来源">${data.sources.map(s=>`<article class="source-card"><div class="author-line">${avatar(s)}<span>${escape(s.author)}</span><span class="type">${s.type==='article'?'文章':'回答'}</span></div><h3>${escape(s.title)}</h3><p>候选来源，观点与语境尚待整理。</p><span class="scope">${escape(s.coverage)}</span><div class="source-actions">${sourceLink(s)}</div></article>`).join('')}</section>${notes()}</div>`;
